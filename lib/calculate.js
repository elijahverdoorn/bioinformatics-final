...
function calculate(topString, sideString, matchScore, mismatchScore, indelScore) {
  ...
  // console.log(mismatchScore);
  // console.log(indelScore);

  matchScore = parseInt(matchScore);
  indelScore = parseInt(indelScore);
  mismatchScore = parseInt(mismatchScore);
  ...
  for (var i = topString.length + sideString.length; i > 0 ; i--){
    ...
    // console.log('node1: ' + node1.southEast);
    // console.log('node2: ' + node2.southEast);
    // console.log('node3: ' + node3.southEast);
		...
		switch (Math.max(node1.southEast, node2.southEast, node3.southEast)) {
			...
			case node3.southEast:
				...
				break;
			default:
				console.log("you dumb, something broke line 241");
				break;
		}

    if (pathArr[pathArr.length - 1].xpos == bestNode.xpos && pathArr[pathArr.length - 1].ypos == bestNode.ypos) {
        pathArr.push(nodesArr[0]);
        break;
    } else {
  ...
  // console.log("\n Path Arr \n", pathArr);
	...
	return({"scoreArr": scoresArr, "nodeArr": nodesArr, "pathArr": pathArr});
}
...
// //Testing
// var testInputString = 'ACG';
// var testInputStringII = 'AGT';
// var useNucleotides = true;
//
// //parseString(testInputString);
// calculate(testInputString,testInputStringII,4,-3,-2);
